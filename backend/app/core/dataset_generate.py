"""Seeded synthetic dataset generator (dCortex crew-ops world).

Deterministic given a seed. Writes the JSON snapshot the rest of the system
loads: flights, crew, rosters, clocks, reserves, certs, rules, costs, risk
signals, scenarios, questions, and held-out scenarios.
"""
...
def _stable_mod(value: str, n: int) -> int:
    digest = hashlib.md5(value.encode(), usedforsecurity=False).hexdigest()
    return int(digest, 16) % n


def generate_dataset(seed: int, out_dir: str | Path) -> Path:
    """Generate a complete dataset into ``out_dir`` and return that path."""
    ...
    def gen_history(cid):
        ...
        for d in HDAYS:
            if on_line and (d.toordinal() % 3 == _stable_mod(cid, 3)) and random.random() < 0.9:
    ...
    # progress is returned via the written files; keep stdout quiet on startup
    ...
    dump("held_out_scenarios.json", held_out)
    dump("manifest.json", {
        "seed": int(seed),
